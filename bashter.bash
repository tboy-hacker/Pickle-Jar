...
function UrlSelector() {
	...
	if [[ ${URL_SELECTOR_MODE} == "y" ]] || [[ ${URL_SELECTOR_MODE} == "Y" ]]; then
		BASESITE=$(echo "${WEBSITE}" | sed 's|/| |g' | awk '{print $2}' | awk -F '.' '{print $(NF - 1)"."$NF}')
	else
		BASESITE=$(echo "${WEBSITE}" | sed 's|/| |g' | awk '{print $2}')
	fi

	if [[ ! -z $(echo "${URL}" | grep ^"javascript:\|mailto:\|tel:\|data:") ]]; then
		echo -ne ""
	elif [[ ! -z $(echo "${URL}" | grep -i "[.]pdf\|[.]doc\|docx\|[.]png\|[.]gif\|[.]jpg\|[.]jpeg\|[.]ico\|[.]svg\|[.]css\|[.]js\|[.]woff") ]]; then
		echo -ne ""
	elif [[ ! -z $(echo "${URL}" | grep ^'//') ]]; then
		if [[ $(echo "${WEBSITE}" | grep ^'https://') ]] && [[ $(echo "${URL}" | grep ^"//[a-zA-Z0-9.]*.${BASESITE}") ]]; then
			echo "[GET] https:${URL}"
		fi
	elif [[ ! -z $(echo "${URL}" | grep ^"/") ]]; then
		...
		FULLURL=$(echo "${WEBSITE}/${URL}" | sed 's|://|P12O70COL|g' | sed 's,/*/,/,g' | sed 's|P12O70COL|://|g')
		echo "[GET] ${FULLURL}"
	elif [[ ! -z $(echo "${URL}" | grep ^"\(http\|https\)://[a-zA-Z0-9.]*.${BASESITE}") ]]; then
		echo "[GET] ${URL}"
	elif [[ ! -z $(echo "${URL}" | grep ^'#') ]]; then
		echo -ne ""
	elif [[ ! -z $(echo "${URL}" | grep ^"[a-zA-Z0-9-]*://" | grep -v ^'http') ]]; then
		echo -ne ""
	else
		echo -ne ""
	fi
...
function CheckForm() {
	...
	IFS=$'\n'
	for FORM in $(perl -nle'print $& while m{<form\K.*?(?=>)}g' "${SOURCECODE}" | sed 's/^/<form/g' | sed 's/$/>/g'); do
		FORMPOST=$(echo $FORM | grep -o 'method=['"'"'"][^"'"'"']*['"'"'"]' | grep -i post)
		if [[ ! -z $(cat ${CHECKEDFORMFILE} 2>/dev/null | grep ''$FORM'') ]]; then
			echo -ne ""
		elif [[ -z ${FORMPOST} ]]; then
			echo "$(date +"[%H:%M:%S]") INFO: Form GET on \"${URL}\""
			echo "$(date +"[%H:%M:%S]") FORM [GET]: \"${URL}\"" >>${HOME_DIR}/scan-logs/${PROC_ID}-info.log
			for FORM_TEST in $(find ${HOME_DIR}/modules/form | grep bash$); do
				if [[ ! -z ${FORM_TEST} ]]; then
...
if [[ -z ${WEBSITE} ]]; then
	...
	read
	exit
elif [[ -z $(echo "${WEBSITE}" | grep ^"\(http\|https\)://") ]]; then
...
if [[ ! -z $(cat ${CURRENT_TEMPFILE} | grep ^'* Could not resolve host:') ]]; then
...
for HEADER_ANALYZE in $(find ${HOME_DIR}/modules/header | grep bash$); do
	if [[ ! -z ${HEADER_ANALYZE} ]]; then
		bash ${HEADER_ANALYZE} "${WEBSITE}" "${CURRENT_TEMPFILE}" "${PROC_ID}"
	fi
done
for URL_ANALYZE in $(find ${HOME_DIR}/modules/url | grep bash$); do
	if [[ ! -z ${URL_ANALYZE} ]]; then
		bash ${URL_ANALYZE} "${WEBSITE}" "${CURRENT_TEMPFILE}" "${PROC_ID}"
	fi
done

echo -ne "" >${URL_LIST}
for URL in $(UrlCrawler ${CURRENT_TEMPFILE}); do
	URLTEMP=$(UrlSelector ${WEBSITE} ${URL} ${CURRENT_TEMPFILE} ${URL_SELECTOR_MODE} | awk '{print $2}')
	if [[ ! -z ${URLTEMP} ]]; then
		if [[ -z $(cat ${URL_LIST} | grep "${URLTEMP}"$) ]]; then
...
CMPCHECKPOINT=0
while true; do
	for WEBSITE in $(cat ${URL_LIST} | awk '{print $2}'); do
		if [[ -z $(cat ${SCANNED_URL} | grep "${WEBSITE}"$) ]]; then
			echo "$(date +"[%H:%M:%S]")(Scan) ${WEBSITE}..."
			LOGNAME=$(echo "${WEBSITE}" | sed 's|/| |g' | awk '{print $2}')
			echo "$(date +"[%H:%M:%S]") URL [Scanned]: \"${WEBSITE}\"" >>${HOME_DIR}/scan-logs/${LOGNAME}-info.log
			CURRENT_TEMPFILE="${TEMPDIR}/"$(echo "SOURCECODE.BASHTER-PROC_$(date +%Y%m%d%H%M%S)${RANDOM}.TMP")
			...
			echo "${WEBSITE}" >>${SCANNED_URL}
			rm ${CURRENT_TEMPFILE}
			if [[ $(cat ${URL_LIST} | wc -l) -eq $(cat ${URL_LIST} | sort -nr | uniq | wc -l) ]]; then
				echo -ne ""
			else
				cat ${URL_LIST} | sort -nr | uniq >${URL_LIST}.XTMP
				cat ${URL_LIST}.XTMP >${URL_LIST}
	...
	if [[ ${CMPCHECKPOINT} -eq ${CMPCOMPARISON} ]]; then
		break
	else
