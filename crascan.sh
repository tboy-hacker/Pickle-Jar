...
luprce() {
	...
	wait
	echo "${f6}[ ${f7}RESULT ON ${f1}rce.txt ${f6}]"
	cat $save3
}
############################################
...
############ Ekse (FILES) ####################
lupfiles() {
	for zfiles in $(cat lib/Files); do
		fast=$(expr $con % $thread)
		if [[ $fast == 0 && $con > 0 ]]; then
			sleep 3
		fi
...
lupjoom() {
	for zjoom in $(cat lib/Components); do
		fast=$(expr $con % $thread)
		if [[ $fast == 0 && $con > 0 ]]; then
			sleep 3
		fi
...
lupadmf() {
	for zadmf in $(cat lib/AdminList); do
		fast=$(expr $con % $thread)
		if [[ $fast == 0 && $con > 0 ]]; then
			sleep 3
		fi
...
######### Shell Scanner ###################
lupshellscn() {
	for zshellscn in $(cat lib/wordlist-shell); do
		fast=$(expr $con % $thread)
		if [[ $fast == 0 && $con > 0 ]]; then
			sleep 3
		fi
